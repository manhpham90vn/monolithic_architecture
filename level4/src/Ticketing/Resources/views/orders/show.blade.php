...
@section('content')
    <div class="card">
        <div class="between">
            ...
            @switch($order->status)
                @case(\Ticketing\Infrastructure\Persistence\OrderEloquentModel::STATUS_PAID)
                    <span class="badge ok">Đã thanh toán</span> @break
                @case(\Ticketing\Infrastructure\Persistence\OrderEloquentModel::STATUS_PENDING)
                    <span class="badge warn">Chờ thanh toán</span> @break
                @case(\Ticketing\Infrastructure\Persistence\OrderEloquentModel::STATUS_EXPIRED)
                    <span class="badge bad">Hết hạn</span> @break
                @case(\Ticketing\Infrastructure\Persistence\OrderEloquentModel::STATUS_CANCELLED)
