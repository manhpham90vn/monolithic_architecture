...
class StoreOrderRequest extends FormRequest
{
    ...
    protected function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            // Ticketing không đọc bảng của Catalog — hỏi qua Public API
            // (QĐ-3.3, QĐ-3.7); route chỉ mang ID sự kiện (QĐ-3.4).
            $eventId = (int) $this->route('event');
            $selected = $this->selectedQuantities();
            $infos = app(CatalogApi::class)->ticketTypeInfos(array_keys($selected));

            foreach (array_keys($selected) as $ticketTypeId) {
                /** @var TicketTypeInfo|null $info */
                $info = $infos[$ticketTypeId] ?? null;

                if ($info === null || $info->eventId !== $eventId) {
