...
it('dispatches OrderPaid so side work is decoupled from the action (QĐ-2.5)', function () {
    EventFacade::fake([OrderPaid::class]);
    [$order] = pendingOrderWithSession();

    $this->postJson(route('stripe.webhook'), completedSessionPayload($order))->assertOk();

    EventFacade::assertDispatched(OrderPaid::class, fn (OrderPaid $event): bool => $event->order->is($order));
});
