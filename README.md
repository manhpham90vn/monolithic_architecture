...
**QĐ-3.1** — Module **BẮT BUỘC** chia theo nghiệp vụ và đặt tên theo ngôn ngữ của người dùng nghiệp vụ ("đơn hàng", "kho", "công nợ" → `Ordering`, `Inventory`, `Billing`). Tên module là `Core`, `Common`, `Helpers` là dấu hiệu chưa hiểu domain — **KHÔNG ĐƯỢC** dùng. (Ngoại lệ duy nhất: `SharedKernel`, xem QĐ-3.9.)

**QĐ-3.2** — Mỗi module **BẮT BUỘC** có ServiceProvider riêng, tự đăng ký route/migration/view và binding Public API của nó:
...
### 6.6. Quy định — transaction và thời điểm phát event
...
### 6.7. Quy định — User và SharedKernel
...
**Cảnh báo trước khi vào mức này:** rất ít dự án Laravel cần mức 4. Nếu domain về cơ bản là "lưu form vào DB rồi hiển thị lại", mức 4 sẽ giết năng suất. Nhiều team đã phải quay lại mức 3 sau 6 tháng.
...
Mở rộng tương tự cho từng cặp module. Chạy `deptrac analyse` trong CI; build đỏ khi có vi phạm.
